...
while True:
    ...
    for char in pipe.read(1):
		...
		if (msg[7] == 0) and not(control & LOCK) and not (control & LINE):		# Left Analog Stick, X-axis
		     ...
		     if throttle > 32640:
			throttle = myMap(throttle, 32641, 64944, 100, 1)
		     elif throttle == 0:
			pass
		     else:
			throttle = myMap(throttle, 1, 32640, -1, -100)
			...
			leftMotor, rightMotor = mixing(throttle, steering)
		     ...
		     if steering > 32640:
			steering = myMap(steering, 32641, 64944, -100, -1)
		     elif steering == 0:
			pass
		     else:
			steering = myMap(steering, 336, 32640, 1, 100)
			...
			steering = myMap(steering, 1, 32640, -1, -100)
		     print('steering = ', steering)
		     # Send throttle and steering data to mixing function
		     rightMotor = steering
		     print('\t', 'Left Motor = ', leftMotor, '  Right Motor = ', rightMotor, '\n')  
